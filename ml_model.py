...
def coeff_determination(y_true, y_pred):
    '''
    custom metric to evaluate performance of the neural network training
    it uses the coefficient of detrmination formula, R^2 = 1 - SS_res/SS_tot
    for more information : https://en.wikipedia.org/wiki/Coefficient_of_determination
    '''
    SS_res =  kb.sum(kb.square(y_true-y_pred ))        # kb is keras backend
...
def training_data(data, panel_data, aoa, re, lift, drag):
    ...
    num_cp = npoints - 1      # number of control points
    nf = data.shape[2]        # number of features
...
'''im = 1: (x,y), 2: (x,y,cl,cd), 3: (x,y,Cp) 4: (x,y,cl)'''
...
# merge_files.py : for merging all airfoil shapes in a single file
# xfoil_analysis_v2.py: for analyzing airfoils and generating CL, CD data using XFoil module in python
...
xtrain2 = np.copy(xtrain[:,2*num_xy:2*num_xy+2])

# xtrain2 = np.zeros((xtrain1.shape[0], 4))
...
for i in range(1,ensembles): #range(1,31)
    ...
    epochs = 30            # the number of times the entire training dataset is passed forward and backward through the neural network
    batch_size = 16       # batch size is the number of samples to be processed before updating the model's parameters (weights and biases)
    
    x1 = Input(shape=(nf1,))
    x2 = Input(shape=(nf2,))
        
    x = Dense(20,activation='relu')(x1)  # the Dense layer is a fully connected layer , relu: f(x) = max (0,x)
...
for i in range(1,ensembles): #range(1,31)
    ...
    xtest_sc2 = np.copy(xtest_sc[:,2*num_xy:2*num_xy+2]) # physics-based features features
    ...
    ax[0].plot(aoa_test[:num_samples], ytest[:num_samples,0], 'ro-', label='Actual')
    ax[0].plot(aoa_test[:num_samples], ypred[:num_samples,0], 'bo-', label='ML')
    ax[0].set_ylabel(r'$C_L$')
    ax[0].set_xlabel(r'$\alpha$')
    ax[0].set_title('Prediction for NACA 23012 Airfoil')
    ax[0].legend()
    
    ax[1].plot(aoa_test[:num_samples], ytest[:num_samples,0], 'ro-', label='Actual')
    ax[1].plot(aoa_test[:num_samples], ypred[:num_samples,0], 'bo-', label='ML')
    ax[1].set_ylabel(r'$C_L$')
    ax[1].set_xlabel(r'$\alpha$')
    ax[1].set_title('Prediction for NACA 0012 Airfoil')
    ax[1].legend()
...
for i in range(1,ensembles): #range(1,31)
    ...
    ax[0].set_ylabel(r'$C_L$')
    ax[0].set_xlabel(r'$\alpha$')
    ax[0].set_title('Prediction for NACA 23024 Airfoil')
    ax[0].legend()
    ...
    ax[1].set_ylabel(r'$C_L$')
    ax[1].set_xlabel(r'$\alpha$')
    ax[1].set_title('Prediction for NACA 0024 Airfoil')
    ax[1].legend()
