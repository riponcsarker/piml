...
for i in range(1, ensembles): #range(1,31)
    ...
    folder_path = 'model_'+str(im)  
    
    # Create the directory, check if it exists first
...
for i in range(1,ensembles): #range(1,31)
    ...
    xtest_sc1 = np.copy(xtest_sc[:,:2*num_xy]) # airfoil shape features
    ...
    ax[0].plot(aoa_test[:num_samples], ypred[:num_samples,0], 'bo-', label='PIML')
    ...
    ax[1].plot(aoa_test[:num_samples], ypred[:num_samples,0], 'bo-', label='PIML')
